 ...
 * Creates a new payment request and returns reference
 */
...
export async function getPaymentStatus(req: Request, res: Response) {
  try {
    ...
    const now = Date.now();
    const expiryTime = payment.createdAt + (15 * 60 * 1000); // 15 minutes
...
export async function mpesaWebhook(req: Request, res: Response) {
  try {
    ...
    // Find payment by amount and recent timestamp (since we don't have reference in callback)
