...
export class MpesaService {
  ...
  async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (data.access_token) {
        ...
        this.tokenExpiry = Date.now() + (parseInt(data.expires_in) - 60) * 1000; // 1 minute buffer
        return this.accessToken;
      }
  ...
  async verifyWebhookSignature(req: Request): Promise<boolean> {
    try {
      ...
      if (!signature) return true; // Allow if no signature (for development)
  ...
  async verifyTransaction(transactionId: string): Promise<{ success: boolean; data?: any }> {
    try {
      const token = await this.getAccessToken();
      const timestamp = new Date().toISOString().replace(/[^0-9]/g, '').slice(0, -3);
      const shortcode = process.env.MPESA_SHORTCODE!;
